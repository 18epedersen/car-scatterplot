...
function drawAxis(car_data) {
  ...
  var h3 = d3.select('body').append('h3').text('D3 Midterm');
  ...
  var horsepower = car_data.reduce(function(acc, cur) {
    if (acc.indexOf(cur.horsepower) === -1) {
      acc.push(cur.horsepower);
    }

    return acc;
  }, []);

  // origin will encode color
  ...
  yScale.range([0, height]).domain(horsepower);

  color.range(d3.schemeCategory20).domain(originTypes);
  // radius.range([0, 15]).domain([0, 10]);
